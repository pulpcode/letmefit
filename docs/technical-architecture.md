...
AI 提取结果不由模型直接写入正式记录。后端会先把模型输出当作候选动作，并默认在当前会话中生成 `pending_action`。客户端在对话框中展示确认卡片，用户可以点击确认、编辑字段、用自然语言补充修正或放弃；只有用户确认后，后端才写入正式记录。
...
工具是否被接受、是否生成确认卡、是否允许确认写入正式记录由后端规则决定。
