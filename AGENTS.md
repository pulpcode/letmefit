...
```text
短信登录 -> 创建档案 -> 记录饮食/身体指标 -> AI 提取 -> 后端规则判定/用户确认 -> 每日归档 -> 总结与建议
```
...
- AI 提取结果必须先成为候选写入动作；明确、低歧义且通过后端规则校验的动作可自动写入正式记录，其余动作必须进入对话中的待确认动作，用户确认或修改后才能写入
...
- 客户端只通过后端 API 读写数据
- 对话中的 Review/Edit 确认状态或确认卡片是模糊、低置信、媒体识别或字段不完整的 AI 结果进入正式记录前的必经步骤
