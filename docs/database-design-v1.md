...
```text
短信登录 -> 创建档案 -> 对话记录 -> AI 生成待确认动作 -> 用户确认/修改 -> 正式记录 -> 每日归档 -> 总结建议
```
...
- AI 提取结果不能直接写入正式记录，必须先进入 `agent_pending_actions`
...
AI 原始提取结果。用于审计、调试和生成待确认动作，不直接等同于正式记录。
...
对话中的待确认动作。它是 Review/Edit 确认卡片的数据来源。
