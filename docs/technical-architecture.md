...
V1 暂不引入 Java 服务。当前阶段的主要任务是验证“低摩擦记录 -> AI 提取 -> 后端规则判定/用户确认 -> 每日建议”的产品闭环，而不是建设多语言微服务体系。
...
AI 提取结果不由模型直接写入正式记录。后端会先把模型输出当作候选动作，再由规则层判断是否可以自动保存：明确数值的身体指标和精确克重餐食可直接写入正式记录；图片识别、模糊描述、低置信度或字段不完整的动作必须先在当前会话中生成 `pending_action`。客户端在对话框中展示确认卡片，用户可以确认、编辑字段、用自然语言补充修正或放弃。
